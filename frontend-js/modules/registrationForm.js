...
export default class RegistrationForm {
  ...
  passwordImmediately() {
    if (this.password.value.length > 50) {
      this.showValidationError(this.password, "password can't exceed 50 char");
    }
  ...
  passwordAfterDelay() {
    if (this.password.value.length < 12) {
      this.showValidationError(
        this.password,
        "Password must be at least 12 char"
      );
    } else {
      this.hideValidationError(this.password);
    }
    ...
    if (!this.username.errors) {
      axios
        .post("/doesUsernameExist", {
         _csrf:this.csrf, username: this.username.value
        })
        .then(response => {
          if (response.data) {
            this.showValidationError(
  ...
  emailAfterDelay() {
    ...
    if (!this.email.errors) {
      axios
        .post("/doesEmailExist", {_csrf:this.csrf,  email: this.email.value })
        .then(response => {
          if (response.data) {
            this.email.isUnique = false;
